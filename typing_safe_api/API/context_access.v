...
3. Others
-----------------------------------------------------------------
- state_new_context : state -> context
...
(* Access keys are position in the telescope which is stable by extension of the
   context with new variables and definitons
*)
Definition key := nat.
...
Section Get.
  ...
  #[local] Definition get_X : state -> key -> X :=
    fun s k => f (#|state_new_context s| - k -1) (get_decl s k).
...
Definition get_pos : state -> key -> nat :=
  fun s k => #|state_new_context s| -k -1.

Definition check_pos: state -> key -> nat -> bool :=
  fun s k read_pos => eqb (#|state_new_context s| -k -1) read_pos.
...
#[local] Definition getp_cdecl : state -> nat -> context_decl :=
  fun s pos => nth pos s.(state_new_context) ERROR_CDECL.
...
Definition check_key : state -> key -> nat -> bool :=
  fun s k pos => eqb (#|state_new_context s| -k -1) (getp_key s pos).
...
Definition reduce_lets : state -> term -> term :=
  fun s t => expand_lets (state_new_context s) t.

Definition reduce_except_lets : state -> term -> term :=
  fun E s t =>
  match reduce_opt noiota_flags Σ (state_new_context s) 5000 t with
  | Some t => t
...
Definition reduce_full : global_env -> state -> term -> term :=
  fun E s t =>
  match reduce_opt default E (state_new_context s) 5000 t with
  | Some t => t
  | None => tVar "ERREUR REDUCTION"
  end. *)