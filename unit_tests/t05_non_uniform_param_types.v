...
Redirect "unit_tests/tests/05_01_nu_list_coq" MetaCoq Run (print_rec "nu_list" ).
Redirect "unit_tests/tests/05_01_nu_list_gen" MetaCoq Run (gen_rec [] <% nu_list %>).

Inductive mixed1 (A B C : Type) : Type :=
| mc11 : mixed1 A B C
| mc12 : mixed1 A nat C -> mixed1 A B C.

Redirect "unit_tests/tests/05_02_mixed1_coq" MetaCoq Run (print_rec "mixed1" ).
Redirect "unit_tests/tests/05_02_mixed1_gen" MetaCoq Run (gen_rec [] <% mixed1 %>).

Inductive mixed2 (A B C : Type) : Type :=
| mc21 : mixed2 A bool C -> mixed2 A B C
| mc22 : mixed2 nat B C -> mixed2 A B C.

Redirect "unit_tests/tests/05_03_mixed2_coq" MetaCoq Run (print_rec "mixed2" ).
Redirect "unit_tests/tests/05_03_mixed2_gen" MetaCoq Run (gen_rec [] <% mixed2 %>).
...
Redirect "unit_tests/tests/05_04_mixed3_coq" MetaCoq Run (print_rec "mixed3" ).
Redirect "unit_tests/tests/05_04_mixed3_gen" MetaCoq Run (gen_rec [] <% mixed3 %>).

Inductive nu_vec (n : nat) : Type :=
| vnil_pa : nu_vec n
| vcons_pa : nu_vec (S n) -> nu_vec n.

Redirect "unit_tests/tests/05_05_nu_vec_coq" MetaCoq Run (print_rec "nu_vec").
Redirect "unit_tests/tests/05_05_nu_vec_gen" MetaCoq Run (gen_rec [] <% nu_vec %>).

Inductive nu_let1 (A : Type) : Type :=
| nu_let1_nil : nu_let1 A
| nu_let1_cons : let x := A in nu_let1 x -> nu_let1 A.

Redirect "unit_tests/tests/05_06_nu_let1_coq" MetaCoq Run (print_rec "nu_let1").
Redirect "unit_tests/tests/05_06_nu_let1_gen" MetaCoq Run (gen_rec [] <% nu_let1 %>).
